...
router.get("/", async function (req, res) {
    var [groups, err] = await model_apps.getAll();
...
router.post('/add_user', async function (req, res) {
    ...
    for (let i = 0; i < items.length; i++) {
        ...
        var [tr, err] = await model.addgroup({
            id_user: userGroup.id_panel_users,
            id_app: element,
...
router.post("/get", async function(req,res) {
    ...
    var [list_group, err] = await model_apps.getAll();
    var groups = []
    console.log("grup user ", users_groups)
    for (let index = 0; index < users_groups.length; index++) {
        const element = users_groups[index];
        groups.push(list_group[element.id_app - 1])
    }
...
router.post('/edit_user', async function (req, res) {
    ...
    for (let i = 0; i < items.length; i++) {
        var element = items[i]
        var [tr, err] = await model.addgroup({
            id_user: req.body.id_panel_users,
            id_app: element,
            id_panel_users: req.user.id
        })
    }
